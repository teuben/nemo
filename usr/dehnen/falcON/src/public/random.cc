...
Sobol::Sobol(const int ACTL, const int BITS)
{
  ...
  if(ACTL>=0 && ACTL < MO)
    ...
    if(actl>=MO)
      falcON_ErrorF("trying to create the 53th object","Sobol::Sobol()");
  }
  f[actl] += 1;
  if(BITS==0)
    bits = setb;
  else if((bits=BITS)<10)
    falcON_WarningF("creating object with less than 10 bits","Sobol::Sobol()");
  in       = 0;
  ix       = 0;
  fac      = 1./(1L<<bits);
  int degs = d[actl];
  ...
  v = falcON_NEW(unsigned long,bits)-1;
...
double Sobol::RandomDouble () const {
  ...
  if(j>bits)
    falcON_ErrorF("trying to call more than 2^BITS times",
		  "Sobol::RandomDouble()");
  ix^= v[j];
  ...
  N(256), N1(N+1), R(r), h(H), hi(1./h), hqi(hi*hi)
{
  register int    i;
  register double y1;
  Y = falcON_NEW(double,N1);
  P = falcON_NEW(double,N1);
  Y[0] = P[0] = 0.;
  Y[N] = P[N] = 1.;
  for(i=1; i<N; i++) {
    Y[i] = i/double(N);
