...
//                                                                              
////////////////////////////////////////////////////////////////////////////////
inline grav_iact::TaylorSeries::TaylorSeries(vect const&x)
  : X(x)
{
...
////////////////////////////////////////////////////////////////////////////////
inline grav_iact::TaylorSeries::TaylorSeries(TaylorSeries const&T)
  : X(T.X)
{
  for(register int i=0; i!=N_COEFF; ++i)
    C[i] = T.C[i];
...
////////////////////////////////////////////////////////////////////////////////
inline void grav_iact::shift_by(real*const&T, vect const&X) const {
  //----------------------------------------------------------------------------
...
#if P_ORDER > 2                                     // 3rd order                
    register vect X3 = third*X;                     //   X/3                    
    SYM2(F2);                                       //   auxiliary Tensor_2     
...
#if P_ORDER > 3                                     // 4th order                
    register vect X4 = quarter*X;                   //   X/4                    
    SYM3(F3);                                       //   auxiliary Tensor_3     
...
#if P_ORDER > 4                                     // 5th order                
    register vect X5 = fifth*X;                     //   X/5                    
    SYM4(F4);                                       //   auxiliary Tensor_4     
...
//------------------------------------------------------------------------------
inline
void grav_iact::extract_grav(TaylorSeries const&T, soul_iter const&S) const
{
  //----------------------------------------------------------------------------
  // computational effort:                                                      
  //                  p   |  1    2    3    4    5    6    7                    
...
//------------------------------------------------------------------------------
inline void grav_iact::shift_to(TaylorSeries&T, vect const&x) const {
  shift_by(T.C,x-T.X);
  T.X = x;
}
//------------------------------------------------------------------------------
inline void grav_iact::shift_and_add(TaylorSeries       &T,
				     vect          const&x,
				     const real*   const&t,
				     real          const&f) const {
  if(t && t[0]) {
				 ...
				 TaylorSeries const&T) const
{
  ...
  LoopSoulKids(cell_iter,C,s) if(is_sink(s)) {      // loop C's sink soul kids  
    s->normalize_grav();                            //     pot,acc/=mass   <    
    extract_grav(G,s);                              //   add pot,acc due to G   
  }                                                 // <                        
  LoopCellKids(cell_iter,C,c) if(is_sink(c))        // loop C's sink cell kids  
    eval_grav(c,G);                                 //   recursive call         
}
//------------------------------------------------------------------------------
void grav_iact::evaluate(cell_iter const&C) const
{
  grav_kern::flush_buffers();                       // finish interactions      
...
//                                                                              
////////////////////////////////////////////////////////////////////////////////
inline void grav_iact_s::eval_grav(cell_iter    const&Ci,
				   TaylorSeries const&T) const
{
  TaylorSeries G(T);                                // G = copy of T            
  shift_and_add(G,cofm(Ci),coeffs(Ci),imass(Ci));   // shift G; G+=T_C          
  take_coeffs(Ci);                                  // free memory: C's coeffs  
  LoopSoulKids(cell_iter,Ci,s) if(is_sink(s)) {     // loop C's sink soul kids  
    s->normalize_grav();                            //     pot,acc/=mass   <    
    extract_grav(G,s);                              //   add pot,acc due to G   
  }                                                 // <                        
  LoopCellKids(cell_iter,Ci,c) if(is_sink(c))       // loop C's sink cell kids  
    eval_grav(c,G);                                 //   recursive call         
}
//------------------------------------------------------------------------------
