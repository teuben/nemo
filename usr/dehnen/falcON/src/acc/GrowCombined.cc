...
namespace {
    ...
    // read line until character `#'; swallow rest of line
  {
	...
	*l=0;
    ...
    error("GrowCombined: line longer than expected\n");
  }
  //----------------------------------------------------------------------------
...
{
  if(npar < 3 || file==0)
    warning("GrowCombined acceleration field recognizes 3 parameters\n"
	    ...
	    NAMAX);
  if(file == 0) error("GrowCombined: not data file given");
  // initialize timer
  timer::index
  ...
  if(!in.good())
    error("GrowCombined: couldn't open file \"%s\" for input\n",file);
  char Line[size], AccName[size], AccPars[size], AccFile[size];
  char*accname=0, *accpars=0, *accfile=0, unknown[9];
  read_line(in,Line,size);
  if(*Line == 0)
    error("GrowCombined: couldn't read data from file \"%s\"\n",file);
  if(strncmp(Line,"accname=",8))
    error("GrowCombined: first entry in file \"%s\" isn't \"accname=...\"\n",
	  file);
  NA     = 0;
  *needM = 0;
  *needV = 0;
  do {
	...
	error("file \"%s\": more accnames than anticipated\n",file);
      ...
      *needV = *needV || nv;
      ...
      if(accpars)
	warning("GrowCombined: additional \"accpars=\""
		" in file \"%s\" ignored\n",file);
      else {
	...
	accpars = AccPars;
      ...
      if(accfile)
	warning("GrowCombined: additional \"accfile=\""
		" in file \"%s\" ignored\n",file);
      else {
	strcpy(AccFile,Line+8);
	accfile = AccFile;
	if(0==strcmp(file,accfile))
	  error("GrowCombined: recursion accfile=datafile not allowed\n");
      }
    } else {
      strncpy(unknown,Line,8); unknown[9]=0;
      warning("GrowCombined: entry \"%s\" in file \"%s\" ignored\n",
	      unknown,file);
    }
  } while(*Line != 0);
}
		  ...
		  char       type)           // I: type: 'f' or 'd'             
{
  ...
  case 'f':
    ...
    case 3: return acc_T<3>(time,nbod,
			    ...
			    static_cast<      float*>(acc),
			    add);
    default: error("GrowCombined: unsupported ndim: %d",ndim);
    }
    break;
  case 'd':
    switch(ndim) {
    ...
    break;
  default: error("GrowCombined: unknown type \"%c\"",type);
  }
}
////////////////////////////////////////////////////////////////////////////////
