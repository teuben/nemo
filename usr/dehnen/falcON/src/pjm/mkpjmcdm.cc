...
// v 4.0    21/10/2005  PJM altered to allow mass as input parameter           |
//-----------------------------------------------------------------------------+
...
#  error mkpjmcdm can only be compiled with proprietary falcON
#endif
...
#include <pjm/pjmCDM.h>                            // his truncated CDM models   
...
namespace {
  ...
  public:
    TruncCDMSampler(double const&__g,              // I: inner power-law slope  
		    ...
		    double const&__m,              // I: maximum v_circ         
      ...
      TruncCDMModel   ( __g, __a,__b,__m, __beta ),
...
void falcON::main() falcON_THROWING
{
  ...
  TruncCDMSampler TS(getdparam("gamma"),
		     getdparam("r_s"),
		     getdparam("r_t"),
		     WD? getdparam("mass")/vf : getdparam("mass"),
		     getdparam("beta")
#ifdef falcON_PROPER
		     ,
		     Rad,nb,
