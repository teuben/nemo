...
#undef CheckInvalid
  ...
  public:
    ...
    ///
    /// \param[out] T    table of bodies::index, sorted
    /// \param[in]  time second argument for func
    /// \param[in]  func function for property to be sorted
...
#define CheckMissingBodyData(B,F) (B)->CheckData((F),__FILE__,__LINE__)
  ...
  public:
    ...
    /// \param[in] zm    reset missing data to zero?
    /// \param[in] warn  warn about missing data fields?
    /// \note If not all data fields required by filter @a F are supported, we
    ///       error out, unless @a zm is true, in which case those data fields
    ///       are assumed zero. If @a warn is true, a warning will be issued
    ///       in this case. In any case, the missing data will only be added
    ///       temporarily.
    void apply_filter(BodyFilter&F, bool zm=false, bool warn=true)
      falcON_THROWING;
    /// apply sort: sort bodies within each bodytype in ascending order of @F
    /// \param[in] F     scalar function to sort
    /// \param[in] keep  which data to keep?
    /// \param[in] zm    reset missing data to zero?
    /// \param[in] warn  warn about missing data fields?
    /// \note If not all data fields required by function @a F are supported,
    ///       we error out, unless @a zm is true, in which case those data
    ///       fields are assumed zero. If @a warn is true, a warning will be
    ///       issued in this case. In any case, the missing data will only be
    ///       added temporarily.
    void apply_sort(BodyFunc<real> const&F, fieldset keep=fieldset::all,
    ...
    bool read_nemo(nemo_in const&Is, fieldset&Read, fieldset Get,
		   const char*range=0, bool warn=1) falcON_THROWING;
    //--------------------------------------------------------------------------
    ...
    fieldset read_part(snap_in const&Si, fieldset Bd, iterator const&start,
		       bool warn=1, unsigned Nread=0) falcON_THROWING;
    //--------------------------------------------------------------------------
    /// Generate a NEMO snapshot on disk from all bodies
    ///
    /// \param[in] Os  nemo output stream
    /// \param[in] Bd  body data fields to be written
    void write_nemo(nemo_out const&Os, fieldset Bd=fieldset::nemo) const
	falcON_THROWING;
    //--------------------------------------------------------------------------
    ...
    void write_nemo(nemo_out const&Os, fieldset Bd, iterator const&start,
		    unsigned Nwrite=0) const falcON_THROWING;
#endif // falcON_NEMO
#ifdef falcON_REAL_IS_FLOAT
    //--------------------------------------------------------------------------
