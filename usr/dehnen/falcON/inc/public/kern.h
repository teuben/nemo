...
namespace nbdy {
  ...
  public:
    ...
    friend bool is_empty(TaylorSeries const&T) {
      return T.C == zero; }
    //--------------------------------------------------------------------------
    inline void shift_and_add(const grav_cell*const&);
    inline void extract_grav (grav_leaf*const&) const;
    //--------------------------------------------------------------------------
  };
  //////////////////////////////////////////////////////////////////////////////
  //                                                                          //
  ...
  protected:
    ...
    //--------------------------------------------------------------------------
  private:
    nbdy::pool           *COEFF_POOL;              // pool for TaylorCoeffs     
  ...
  public:
    struct acl_block {
      ...
      vect             dX[4];
      fvec4            D0,D1;
      void load_g(grav_cell*const&a, grav_leaf*const&b, vect&dR, real const&Rq)
      {
	A [NR] = a;
	B [NR] = b;
	dX[NR] = dR;
...
#ifdef falcON_INDI
      fvec4            EQ;
      void load_i(grav_cell*const&a, grav_leaf*const&b, vect&dR, real const&Rq)
      {
	A [NR] = a;
	B [NR] = b;
	dX[NR] = dR;
    ...
    struct acc_block {
      ...
      vect             dX[4];
      fvec4            D0,D1;
      void load_g(grav_cell*const&a, grav_cell*const&b, vect&dR, real const&Rq)
      {
	A [NR] = a;
	B [NR] = b;
	dX[NR] = dR;
...
#ifdef falcON_INDI
      fvec4            EQ;
      void load_i(grav_cell*const&a, grav_cell*const&b, vect&dR, real const&Rq)
      {
	A [NR] = a;
	B [NR] = b;
	dX[NR] = dR;
  ...
  protected:
    grav_kern_base(
      ...
      QQ         ( quarter * EQ ),
#endif
      COEFF_POOL ( falcON_Memory(new nbdy::pool(max(4u,np),
						grav::NCOEF*sizeof(real))) ),
      NC         ( 0 ),
      MAXNC      ( 0 ) {}
    //--------------------------------------------------------------------------
    ~grav_kern_base() {
    ...
    //--------------------------------------------------------------------------
#define ARGS__ grav::leaf_iter const&, uint const&, 	\
               grav::leaf_iter const&, uint const&
