...
#define falcON_included_numerics_h
...
namespace falcON {
			 ...
			 const scalar_type x)
  {
    ...
    register int i,m;
    register num_type y,*P=falcON_NEW(num_type,n);
...
#define PRNGE static_cast<range*>
  ...
  private:
    typedef falcON::block_alloc<range> ranger;     // allocator for new ranges  
    //--------------------------------------------------------------------------
  ...
  template<typename sortit>
    sorttree(sortit const&A,
	     ...
	     scalar const&xmin,
	     scalar const&xmax) :
      Ncrit ( falcON::max(1,nc) ),
      RG    ( falcON::max(10,n/Ncrit) ),
      ROOT  ( RG.new_element() ),
      L0    ( falcON_NEW(leaf,n) )
    {
    ...
    //--------------------------------------------------------------------------
    void merge(const sorttree* const that) falcON_THROWING {
      if(that->Ncrit != this->Ncrit)
	falcON_THROW("sorttree::merge(): Ncrit differs: cannot merge");
      if(that->L0==0 || this->L0==0)
	falcON_THROW("sorttree::merge(): leafs deleted: cannot merge");
      if(that->ROOT->C != this->ROOT->C ||
	 that->ROOT->S != this->ROOT->S)
	falcON_THROW("sorttree::merge(): ranges differ: cannot merge");
      Ncrit;
...
////////////////////////////////////////////////////////////////////////////////
#endif // falcON_included_numerics_h
