...
// C++ code                                                                    |
//                                                                             |
// Copyright Walter Dehnen, 1994-96, 2004                                      |
// e-mail:   walter.dehnen@astro.le.ac.uk                                      |
// address:  Department of Physics and Astronomy, University of Leicester      |
//           University Road, Leicester LE1 7RH, United Kingdom                |
//                                                                             |
//-----------------------------------------------------------------------------+
#include <proper/PotExp.h>
...
namespace {
  ...
  public:
    ...
    CasRec(int l) : L(l), D(new scalar[l+l+1]), A(l+D) {}
    ~CasRec() { delete[] D; }
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // element access                                                           
  ...
  public:
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // construction & destruction                                               
    YlmRec(int l)          : L(l),   L1(L+1), L1Q(L1*L1), A(new scalar[L1Q]){}
    YlmRec(YlmRec const&a) : L(a.L), L1(L+1), L1Q(L1*L1), A(new scalar[L1Q]){
  ...
  public:
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // construction & destruction                                               
    AnlRec(int n, int l) : N1(n+1), L1(l+1), A( new scalar[N1*L1]) {}
    AnlRec(AnlRec const&a) : N1(a.N1), L1(a.L1), A( new scalar[N1*L1]) {
  ...
  //////////////////////////////////////////////////////////////////////////////
  //                                                                            
  // static variables to hold info about scale radius used by static methods    
  //                                                                            
  //////////////////////////////////////////////////////////////////////////////
  scalar R0, IR0;
#ifdef falcON_SSE
  fvec4  IR04;
#endif
  inline void setR0(scalar const& r0) {
    R0   = r0;
    IR0  = 1/r0;
#ifdef falcON_SSE
    IR04 = IR0;
  ...
  //----------------------------------------------------------------------------
#ifdef falcON_SSE
#  if defined (__INTEL_COMPILER) || defined(__PGCC__)
#    define LVAL(X) (X[0])
#  else
#    define LVAL(X) (X)
#  endif
  ...
  //----------------------------------------------------------------------------
  template<typename T>
  inline void Spherical4(fvec4&rd,
			 fvec4&ct, fvec4&st,
    ...
    __asm__ volatile ("movaps  %0,     %%xmm0\n\t" /* xmm0: x               */ \
			...
			"=m" LVAL(ct), "=m" LVAL(st),			       \
			"=m" LVAL(cp), "=m" LVAL(sp)			       \
		      : "m" LVAL(y), "m" LVAL(z), "m" LVAL(IR04) );
    for(int i=0; i!=4; ++i)
      if(x[i] == 0) {
	ct[i] = z[i] > 0? scalar(1) : scalar(-1);
	st[i] = 0;
	cp[i] = 1;
	sp[i] = 0;
      }
#else
    for(int i=0; i!=4; ++i)
      Spherical(rd[i],ct[i],st[i],cp[i],sp[i], X[i]);
#endif
  }
  //----------------------------------------------------------------------------
  ...
  // in place (ax,ay,az), erasing (ar,ath,aph)                                  
  template<typename X>
  inline void Cartesian4(tupel<3,X>*a,
			 fvec4 const&ct, fvec4 const&st,
			 fvec4 const&cp, fvec4 const&sp) {
#ifdef falcON_SSE
    fvec4
      a0(a[0][0], a[1][0], a[2][0], a[3][0]),
      a1(a[0][1], a[1][1], a[2][1], a[3][1]),
    ...
    __asm__ volatile ("movaps  %0,     %%xmm0\n\t" /* xmm0 <- ar            */ \
		      ...
		      "movaps  %%xmm0, %1    \n\t" /* xmm0 -> ay            */ \
		      "movaps  %%xmm4, %2    \n\t" /* xmm4 -> az            */ \
		      : "=m" LVAL(a0), "=m" LVAL(a1), "=m" LVAL(a2)	       \
		      : "m"  LVAL(ct), "m"  LVAL(st),			       \
		        "m"  LVAL(cp), "m"  LVAL(sp)  );
    a[0][0]=a0[0]; a[1][0]=a0[1]; a[2][0]=a0[2]; a[3][0]=a0[3];
    a[0][1]=a1[0]; a[1][1]=a1[1]; a[2][1]=a1[2]; a[3][1]=a1[3];
    a[0][2]=a2[0]; a[1][2]=a2[1]; a[2][2]=a2[2]; a[3][2]=a2[3];
#else
    for(int i=0; i!=4; ++i)
      Cartesian(a[i],ct[i],st[i],cp[i],sp[i]);
#endif
  }
}
////////////////////////////////////////////////////////////////////////////////
...
namespace {
  ...
  class GBlock {
    ...
    void AddGravity(                               // add gravity to single body
      ...
      SetPsi<SYM>(Psi,dPr,rd[0]);                  //   set Psi, dPsi/dr        
