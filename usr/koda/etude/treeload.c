/****************************************************************************/
/* TREELOAD.C: routines to create tree.                                     */
/* Public routines: planttree(), managetree()                               */
...
/*
 * PLANTTREE: construct tree structure for hierarchical force calculation.
 */

void planttree(bodyptr btab, int nbody)
{
    double cpustart;
    bodyptr p;
...
/*
 * MANAGETREE: compute center-of-mass and link tree branches.
 */

void managetree(void)
{
    double cpustart;
    int i;
...
local void loadbody(bodyptr p)
{
	...
	if (++lev > MAXLEVEL)                   /* check tree depth         */
	    error("loadbody: tree depth exceeds maximum\n");
