...
odeint (ystart, nvar, x1, x2, eps, h1, hmin, nok, nbad, derivs, rkqc)
...
int    (*derivs)(), (*rkqc)();		/* pointers to functions */
{
...
#if defined(DEBUG)
      printf ("ODEINT: x1,x2,eps,h1,hmin=%f %f %f %f %f\n",x1,x2,eps,h1,hmin);
#endif
      x=x1;			/* start integrating here */
      h=sign(h1,x2-x1);		/* with this initial stepsize */
#if defined(DEBUG)
      printf (" ...  starts at x=%f   with h=%f\n",x,h);
#endif
	...
	for (i=0; i<nvar; i++) 
...
#if defined(DEBUG)
         printf ("ODEINT: y,h,dydx,yscal=%f %f %f %f\n",y[0],h,dydx[0],yscal[0]);
#endif
        ...
        if (fabs(hnext) < hmin) 
           printf ("ODEINT: Stepsize smaller than minimum.");
        h=hnext;
      }
      printf ("ODEINT: Too many steps.\n");
      return(0);
}
