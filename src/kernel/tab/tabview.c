/*
 *  TABVIEW: prototype implemention of dynamic dynamic queries
 *	     for ASCII tables
 *		Also prints out various low life statistics ....
 *
...
interact()
{
	...
	/* gets() is dangerous and should not be used */
        ...
        switch (line[0]) {
            ...
            case '7':
            case '8':
            case '9':  itmp = atoi(line)-1; 
                       if (itmp>=0 && itmp<nsliders)
                          s = itmp;
                       else
                          warning("Not a legal slider");
                       break;
            case '!':  if (line[1]) strcpy(cmd_system, &line[1]);
                       if (cmd_system[0]==0) break;
	               printf("%s\n",cmd_system);
                       system(cmd_system);
                       break;
            case '|':  if (line[1]) strcpy(cmd_popen,&line[1]);
            ...
            case 'h':  if (*word(line,2) == 's') {
                       ...
                       break;
	    case 'b':  sliders[s].step = atof(word(line,2));
                       sliders[s].stepper = 3;
                       break;
            case 's':  printf("\n");
                       for (i=0; i<nsliders; i++) 
                       ...
                       printf("\n");
                       break;
            case 'o':  table_output(word(line,2));
                       break;
            case 'r':  sliders[s].shi = sliders[s].smax;
            ...
            case '?':  printf("%s\n",interact_help);
                       ...
                       break;
            case 'q':  return -1;
            case 'x':  rtmp = sliders[s].slo;
                       sliders[s].slo = sliders[s].shi;
                       sliders[s].shi = rtmp;
                       done = TRUE;
                       break;
            case 'f':  while(pl_cursor(&xcm, &ycm, &c)) {
                          if (c=='X') break;
...

re_display(k)       /* redisplay all, or slider 'k' (1..nsliders) */
int k;
{
