/*
 * YAPP_MESA.C: Yet Another Plotting Package -- MESA 3D - almost GL
 *
 *	10-dec-99    PJT    --  at SGI's university :-)  --
 *
 */
...
/*	mesa 3d stored their header files in /usr/include/GL      */
#include <GL/gl.h>
...
plstop()
{
  ...
  /* ---  these don't appear to exist in MESA
    qdevice(KEYBD);
...
plltype(int lwid, int lpat)
{
    if (yappstat == INACTIVE)
	error(inmsg);
    if (lwid)  glLineWidth((float)lwid);
    if (lpat)  glLineStipple(lpat,255);
}
