...
string defv[] = {
  ...
  "iscale=1       Scale image values [not implemented]",
  "VERSION=1.0\n  17-aug-2012 PJT",
  NULL,
};
...
real efactor(string u1, string u2)
{
  ...
  if (streq(u1,"AU")) {
    ...
    warning("u1=pc not implemented");
  } else if (streq(u1,"km/s")) {
    if (streq(u2,"km/s")) 
      s = 1;
    else if  (streq(u2,"m/s")) 
      s = 1e-3;
    else
      warning("Comparison unit %s not understood for %s",u2,u1);      
