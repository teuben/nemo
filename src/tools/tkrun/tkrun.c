...
#define VERSION_ID  "3-aug-2023"

				/* tcl/tk startup code */
char *preamble=\
    "#! /usr/bin/env -S wish -f\n" 
    "#! /usr/bin/wish -f\n"      // alternatively:   /usr/bin/env -S wish -f
...
int parse(FILE *fp, FILE *tp, FILE *sp, int maxw, Widget *w)
{
    ...
    /*        [set] key=val   # HELP    #> GUI       */

    while (fgets(line,MAXLINELEN,fp) != NULL) {
        len = strlen(line);
        if (line[len-1]=='\n') line[len-1]='\0';
 
        if (strncmp(line,keycdef,cdeflen)==0) {    /* if line starts with "#<"  - old style V1 */
            cp = strchr(line,'-');
            if (cp) {
                if (nw==0) continue;
                if (w[nw-1].opt) 
		...
		// new
                ...
                if (strcmp(wp[1],"IFILE")==0)
                    ...
                    fprintf(sp,"   set %s [tk_chooseDirectory -parent %s]\n",
                            tvar,tval);
                else if (strcmp(wp[1],"ODIR")==0)    // @todo fix this to be ODIR, not IDIR
                    fprintf(sp,"   set %s [tk_chooseDirectory -parent %s]\n",
                            tvar,tval);
