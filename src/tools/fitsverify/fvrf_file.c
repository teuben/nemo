...
/* ffmbyt is an internal CFITSIO routine ; the prototype is in fitsio2.h */
...
{   
   ...
   status = 0;  
   fits_clear_errmsg();
   if(fits_get_hduaddr(infits, &headstart, &datastart, &dataend, &status)) 
   ...
   {
       sprintf(errmes, 
   "Error trying to read last byte of the file at byte %ld.", dataend);
       wrterr(out,errmes,2);
       wrtout(out,"< End-of-File >");
       wrtout(out," ");
       return;
   ...
   if(status == 0) { 
       wrtout(out,"< End-of-File >");
       sprintf(errmes, 
     "File has extra byte(s) after last HDU at byte %ld.", dataend);
       wrterr(out,errmes,2);
       wrtout(out," ");
   } 
