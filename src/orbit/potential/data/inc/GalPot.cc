...
inline void find(int& klo, const int n, S *x, const S xi)
{
	...
	klo = hunt(x,n,xi,klo);
        if(klo<0 || klo>=n) { std::cerr<<"find(): x out of range"<<'\n';
		    	      std::exit(1); }
    }
}
...
double DiskAnsatz::mass_integrand(const double y) const
{
    if(y<=0. || y>=1.) return 0.;
...
Disks::Disks(istream& from)
{
    if(!from) {
        std::cerr<<" Trying to construct Disks from a closed istream\n";
        std::exit(1);
    }
    DiskPar P;
    from >> nd;
...
Spheroids::Spheroids(istream& from)
{
    if(!from) {
...
const int N =LMAX/2+1,	// number of multipoles
	  N2=3*N/2,	// number of grid point for cos[theta] in [0,1]
	  N4=5*N/2; 	// number of points used to integrate over cos[theta]

typedef Tupel<double,N> DBN;
                      ...
                      PotResidual const *PR)
{
    ...
    register double dx,dx2,xl_ll,xh_ll,risq,ril2,dP;
             DBN    *rhol,*rhl2,*W,*P1,*P2,A[4],P2l,dP2l,*Phil,*dPhl;
...
//
    if(beta>0. && beta<3.) {
        std::cerr<<" Warning: beta= "<<beta<<" unsuitable for Multipole expansion;"
            <<" we'll take beta=3.2\n";
        beta=3.2;
    }
//
