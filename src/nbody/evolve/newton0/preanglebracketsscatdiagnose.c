...

#define  INVERSE_FREQUENCY_OF_DEBUG_OUTPUTS  50
...

/*-----------------------------------------------------------------------------
 *  is_new_node  --  if this particular tuple is not isolated, or does not form
...
int  k;
    ...
    if (is_bound_tuple(tuple, nodes, dist_table, k) == FALSE)
	return(FALSE);
 ...
 *-----------------------------------------------------------------------------
 */
local bool  is_bound_tuple(tuple, nodes, dist_table, k)
int  tuple[NBODY_MAX];
bodyptr  nodes;
real  dist_table[NNODE_MAX][NNODE_MAX];
int  k;
    ...
    tuple_energy = tuple_pot_energy(tuple, nodes, dist_table, k)
	           + tuple_kin_energy(tuple, nodes, k);
...
#define   VERY_MUCH_LARGER_THAN_ONE        1.e9
 ...
 */
    ...
    local_separation_criterion = VERY_MUCH_LARGER_THAN_ONE;
    local_energy_criterion = VERY_MUCH_LARGER_THAN_ONE;
    global_energy_criterion = VERY_MUCH_LARGER_THAN_ONE;
/*
...

    ...
    if (debug == TRUE && min_halt_criterion > DEBUG_HALTING_SPEEDUP_FRACTION)
	{
 ...
 *              note:
 *                    for a k_tuple with  k > 3  the instantaneous radius
 *                    is computed; 
...
int  k;
    ...
    update_admin_table(tuple, *nnodeptr, admin_table, struct_table, k);
 ...
 *-----------------------------------------------------------------------------
 */
local void  update_admin_table(tuple, nnode, admin_table, struct_table, k)
int  tuple[NBODY_MAX];
int  nnode;
int  admin_table[NNODE_MAX][N_ADMIN];
real  struct_table[NNODE_MAX][N_STRUCT];
