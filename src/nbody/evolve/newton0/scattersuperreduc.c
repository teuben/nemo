...
/*
 *  scattersuperreduc.c: as scatterreduc, but more terse
 *
...
main()
    ...
    char  veryanc[ MAX_BLOCK_LENGTH ];             /* very ancient */
    int  getblock();
    bool  isstartblock();
    bool  unexpected();
    bool  interesting();

    ...
    getblock(veryanc, MAX_LINE_LENGTH, MAX_BLOCK_LENGTH);
	...
	if (interesting(new, &how_many_before, &how_many_after) == TRUE)
		...
		print_next = TRUE;
	    if (how_many_after > 1)
		print_afternext = TRUE;
	    else
		print_afternext = FALSE;
	    }
	else
	    print_afternext = FALSE;

	if (print_veryanc == TRUE)
	    {
	    printblock(veryanc);
	    skipping = FALSE;
	    }
...

	copyblock(veryanc, anc);
	copyblock(anc, old);
	copyblock(old, new);
	print_veryanc = print_anc;
	print_anc = print_old;
	print_old = print_new;
	print_new = print_next;
	print_next = print_afternext;
	}
