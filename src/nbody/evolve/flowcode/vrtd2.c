/* 
 * VRTD2: flow potential (and density weighed interpolated forces) from an image :
 *      2D version, from Gilberto's 0->pi half plane simulations at Z=0
...
void inipotential (int *npar, double *par, string name)
{
    ...
    warning("vrtd2.so:: THIS CODE HAS NOT BEEN CERTIFIED");
...
void potential(int *ndim,double *pos,double *acc,double *pot,double *time)
{
    ...
    y = pos[1];
    rad = sqrt(x*x + y*y);
    phi = atan2(y,x);               /* make sure phi in 0 : pi  */
