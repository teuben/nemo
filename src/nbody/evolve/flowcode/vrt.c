/* 
 * VRT: flow potential (and interpolated forces) from an image : 2D version
 *      z-forces are merely returned as 0.0
 *	4 images are expected: 
 *          2D vr
 *          2D vt
...
#define CCD_VERSION "flowcode:vrt V1.0 16-apr-96"
...
local imageptr vr = NULL, vt = NULL, r = NULL, t = NULL;
local int      nr, np;
...
void inipotential (int *npar, double *par, string name)
{
    ...
    if (n) n = read_image (potstr,&r);              /* read the 3rd image */
    if (n) n = read_image (potstr,&t);              /* read the 4th image */
    ...
    if (Nx(vt) != nr)  error("incorrect R dimension of 2nd image");
    if (Ny(vt) != np)  error("incorrect T dimension of 2nd image");
    if (Nx(r)  != nr)  error("incorrect R dimension of 3rd image");
    if (Ny(r)  != 1)   error("incorrect 1 dimension of 3rd image");
    if (Nx(t)  != np)  error("incorrect T dimension of 4th image");
    if (Ny(t)  != 1)   error("incorrect 1 dimension of 4th image");
...
void potential(int *ndim,double *pos,double *acc,double *pot,double *time)
{
    ...
    if (rad > rads[nr-1] || rad < rads[0]) {
        ...
        a1 = VR(ir,ip)*c1 + VR(jr,ip)*c2;
        a2 = VR(ir,jp)*c1 + VR(jr,jp)*c2;
        vrad = a1*c3 + a2*c4;

        a1 = VT(ir,ip)*c1 + VT(jr,ip)*c2;
        a2 = VT(ir,jp)*c1 + VT(jr,jp)*c2;
        vtan = a1*c3 + a2*c4;
