...
*       Initialize the portable random number generator (range: 0 to 1).
      ...
      A(1) = RAN2(KDUM)
*
*       Skip the first random numbers (NLIST(1) specified at input).
      NRAND = NLIST(1)
      DO 5 K = 1,NRAND
          A(1) = RAN2(KDUM)
    5 CONTINUE
...
*
*       Include the case of equal masses (ALPHAS = 1.0 or BODY1 = BODYN).
...
*       Scale coordinates & velocities to analytical expectation values.
      ...
      DO 88 I = 1,N
          DO 86 K = 1,3
              ...
              X(K,I) = A(1)*X(K,I)
              XDOT(K,I) = A(2)*XDOT(K,I)
   86     CONTINUE
   88 CONTINUE
*
*       Check optional input of binary orbit.
   90 IF (KZ(12).EQ.0) GO TO 100
*
      READ (5,*)  SEMI, ECC
      VREL = SQRT (ZMASS*(1.0 - ECC)/(SEMI*(1.0 + ECC)))
      DO 98 I = 1,N
          DO 95 K = 1,3
              X(K,I) = 0.0
              XDOT(K,I) = 0.0
