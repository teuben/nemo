...
*       Note: D2 now stored as D2/18 for GRAPE-6. 
      DO 15 L = 2,NNB2
          ...
          S2 = 2.0*S
          S4 = 0.75*S
          X(1,J) = (((D2(1,J)*S4 + FDOT(1,J))*S + F(1,J))*S +
     &                                           X0DOT(1,J))*S + X0(1,J)
          X(2,J) = (((D2(2,J)*S4 + FDOT(2,J))*S + F(2,J))*S +
     &                                           X0DOT(2,J))*S + X0(2,J)
          X(3,J) = (((D2(3,J)*S4 + FDOT(3,J))*S + F(3,J))*S +
     &                                           X0DOT(3,J))*S + X0(3,J)
	  XDOT(1,J) = ((D2(1,J)*S + FDOT(1,J))*S1 + F(1,J))*S2 +
     &                                           X0DOT(1,J)
	  XDOT(2,J) = ((D2(2,J)*S + FDOT(2,J))*S1 + F(2,J))*S2 +
     &                                           X0DOT(2,J)
	  XDOT(3,J) = ((D2(3,J)*S + FDOT(3,J))*S1 + F(3,J))*S2 +
     &                                           X0DOT(3,J)
   15 CONTINUE
      END IF
*
