...
*       Form the corresponding energy change.
      ...
      DE(2) = 0.0
*
*       Correct the energy (may not be needed with new EJUMP algorithm).
      CALL CONST(X,V,M,N,EN1,G0,ALAG)
      ERR = EN1 - ENERGY
      WRITE (6,1)  ERR, EJUMP
    1 FORMAT (' QPMOD    CORRECT    ERR EJUMP  ',1P,2E10.2)
      EJUMP = 0.0
      ENERGY = ENERGY + ERR
*
...
*       Derive scale factor from velocity ratio for chaotic motion.
      IF (KZ27.GT.1.AND.KSTARI.EQ.-1) THEN
          C2 = (H + MB/PERI1)/(HI + MB/PERI)
...
*       Activate indicator for synchronous orbit.
      IF (ECC.GT.0.002.AND.ECC1.LT.0.002) THEN
          ISYNC = 1
      END IF
*
