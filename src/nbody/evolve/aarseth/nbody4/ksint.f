...
*       Terminate if apocentre perturbation > 0.25 (R > SEMI) or GI > 0.25.
          IF (HI.LT.0.0) THEN
              ...
              IF (RI.GT.5.0*RMIN.AND.GI.GT.GMAX) IQ = .TRUE.
          ELSE
      ...
      END IF
*
*       Check termination of wide binary near perturber limit.
      IF (RI.GT.5.0*RMIN.AND.NNB0.GT.0.8*LMAX) IQ = .TRUE.
*
...
*             CALL EVOLVE(IPAIR,K)
*         END IF
*       Allow weakly perturbed energetic mergers.
          IF (NAME(I).LT.0) THEN
...
*       Update termination length scale in case of initial soft binary.
      ...
      ELSE 
          ...
          ECC1 = SQRT(ECC2)
          RP = SEMI*(1.0 - ECC1)
          IF (RP*(1.0 - GI).LT.2.0*R0(IPAIR)) THEN
              Q = BODY(I2)/BODY(I)
...
*       Find merger index.
   ...
   72         CONTINUE
              ...
              E0 = 0.0
              EMAX = 0.0
*       Obtain an improved triple stability boundary inside 1.5*RP.
              IF (PCRIT.LT.4.0*RP.AND.NAME(I2).LE.NZERO) THEN
                  RIM = 0.0
                  TD2 = 0.0
*       Form inner eccentricity from KS variables.
   ...
   73             CONTINUE
                  ...
                  E0 = SQRT(E02)
*       Determine maximum eccentricity, inclination and stability boundary.
                  CALL HIMAX(I1,IM,E0,AIN,EMAX,EMIN,ANGLE,TG,EDAV)
*       Note: updating XREL & VREL in HMDOT should not affect inclination.
                  PCRIT = stability(CM(1,IM),CM(2,IM),BODY(I2),E0,
     &                                             ECC1,ANGLE)*AIN
*       Save the revised stability boundary for later use.
                  R0(IPAIR) = PCRIT
              END IF
              IF ((RP*(1.0 - GI).LT.R0(IPAIR).AND.E0.EQ.0.0D0).OR.
     &            (RP.LT.0.6*PCRIT.AND.GI.LT.0.005))  THEN
