...
*       Note that A1(J,K) is the transpose of A1(K,J).
   ...
   30 CONTINUE
*
*       Evaluate initial binding energy per unit mass and T0.
      H(IPAIR) = (2.0D0*(UDOT(1,IPAIR)**2 + UDOT(2,IPAIR)**2 +
     &                   UDOT(3,IPAIR)**2 + UDOT(4,IPAIR)**2) -
     &                                              BODY(ICM))/R(IPAIR)
*
*       Specify dummy perturber for correct treatment in FPOLYI (h2setmj).
      LIST(1,2*IPAIR-1) = 1
      T0(2*IPAIR-1) = TIME
*
...
*       Exit after initialization.
      GO TO 100
*
*       Obtain force polynomial for c.m. on GRAPE (irregular part on host).
   50 IPH = IPHASE
      IPHASE = 0
      CALL FPOLYI(ICM)
      IPHASE = IPH
...
*
     ...
     &              (X(3,ICM) - RDENS(3))**2)
          ...
          WRITE (6,60)  WHICH1, TTOT, NAME(ICOMP), NAME(JCOMP),
...
*       Check diagnostic output of new hard binary.
     ...
     &              (X(3,ICM) - RDENS(3))**2)
          IF (IPHASE.EQ.6) K = -1
          WRITE (8,90)  TTOT, NAME(ICOMP), NAME(JCOMP), K, BODY(ICOMP),
