/*
 * SNAPPLOT.C: plot particle positions from a snapshot output file;
 *	       will plot trajectories instead if invoked as trakplot.
 *	V2.2  22 jan 89 - some version - JEB
 *	V2.2a fixed plcircle argument for when yapp_ps_new is used - apr 89
 *	V2.2b 22 may 92 - helpvec, usage	PJT
 *        also added optional reading ascii colortables (ala Gipsy; 
 *        see $NEMODAT/lut)
 *	V2.2c repaired bug in suntools: it would call scrdump()
 *            due to another global symbol 'frame'; made all global variables
 *            'local' (static), except defv/usage of course.
 *            Also changed all malloc's to allocate                 PJT
 *      V2.2d point (when psize < 0) become crosses (X) when fill_circle=false
 *            and remain plusses (+) when fill_circle=true (the default)   PJT
 *	  d-e added nxy= to test multiple panels, took away -DSCIAM       pjt
 *      V3.0  renamed major version since pjt's 'snaplotedit' version
 *	      is at V2.7 or so.						  pjt
 *	V3.1  don't do first time plframe() for interactive devices       pjt
 *	V3.2  support for SINGLEPREC using proper prototypes              pjt
...
local stream instr;
local string xvar, yvar;
local btrproc xfunc, yfunc;
...
local real xrange[3], yrange[3], crange[3];
