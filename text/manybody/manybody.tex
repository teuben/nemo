...
Christian Boily for the first school in Strasbourg.
...
under peculiar conditions (see Salmon \& Warren 1993).
...
\smallskip
There are some C++
compiler issues with this code, though  gcc 3.2.2 is known to work well.
...
Salmon, J.K. \& Warren, M.S. - 1993 -
{\it Skeletons from the Treecode Closet} J. of Comp. Phys.: {\bf skeletons.ps}
...
\item
{\tt firefox /home/blabla/manybody/index.html} : local HTML 
...
\section{blabla.astro.uva.nl}

The directory {\tt /home/blabla/manybody} on  {\tt blabla.astro.uva.nl} 
...
Much of the available open source software we use, and even those who don't, use
the {\tt autotools} set to {\tt configure}, {\tt make} and {\tt install} their
...
and {\tt /usr/local}, {\tt /opt} or {\tt /sw} (e.g. Fink) are used. As long
...
So, for almost all simple packages we can use the kinds of commands
...
and for packages that depend on these, 
...
fltk	   1.1.4
hdf        4.2r0
plplot     5.x          http://plplot.sourceforge.net/   
pgplot     5.5.2 (via cvs.astro.umd.edu)
gsl        1.4
...
\item
data is mostly stored in special ascii files, which a hierarchical structure
