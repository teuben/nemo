...
\date{{\small Version 1.1} \\
      {\small Nearly-Spring 2004} \\
...
the ``other'' software you will hopefully also find useful this weekend.
Although
the main material from the book {\it Moving Stars Around}
...
and also made available on the Strasbourg linux 
cluster for the course of 
this school\footnote{see {\tt /home/instructeur/manybody}}.

Most of this software are the NEMO  and Starlab packages, though
a number of useful programs and smaller packages
...
story that you should read front to back, it is more encyclopedic
and will hopefully give you an impression of the capabilities
of this software in case you may need it during the weekend or in
your subsequent research. On most topics in this guide you can find
more in-depth information in other manuals and of course 
...
of Unix. Most examples show simple snippets
...
as Starlab programs.
For NEMO programs the option {\tt help=h} will give more extensive help
...
http://astro.u-strasbg.fr/scyon/School/     this N-body School
...
\begin{verbatim}
    source /home/instructeur/manybody/manybody_start
\end{verbatim}
...
\item
{\tt mozilla /home/instructeur/manybody/index.html} : local HTML 
...
\section{ncorps.u-strasbg.fr} 

The directory {\tt /home/instructeur/manybody} on  {\tt ncorps.u-strasbg.fr} 
...
First a overview of the {\it manybody} hierarchy:
...
it is worth reading
this page and considering to use it (a few packages
in manybody are CVS enables, encouraging you to use this feature).
...
macros. Most of this is written up well in the 
...
\end{verbatim}\normalsize
controlled with the {\tt error=} and {\tt warning=} system keywords.
...
\item
The default shell must be csh, only limited (ba)sh support. Command: {\tt chsh}.
...
\item
The {\tt gman} appears to be missing on {\tt ncorps}
...
\item
dtos suffers from some quick hacks put in for the AAS NVO demo:
Aux/Acceleration/Potential are meaningless.

\item
(run)galaxy (sellwood) has problems with post-run dataconversion. 
unfio has changed interface.

\item
GalactICS has not been properly checked 

\item
snapplot multipanel has odd colors (grey) showing up
