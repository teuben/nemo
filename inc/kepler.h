...
#define   PPhsp(partptr)             (&(partptr)->part_space)
...
#define   KPhsp(kepptr)              (&(kepptr)->kep_space)
...
/*-----------------------------------------------------------------------------
 *  scopy  --  copies an arbitrary structure '*a'to a structure '*b'
 *	       of the same type.
 *	       note: if the compiler does not allow structure assignment,
 *		     compile with the    cc -OLDCC   option.
 *		     by default structures are assigned simply with '=' .
 *	       invokes: scopyf() which resides in "stdinc.c"
 *-----------------------------------------------------------------------------
 */
#ifdef  OLDCC
#define     scopy(a,b)     scopyf((char *)(a), (char *)(b), sizeof(*a))
#else
#define     scopy(a,b)     (*(a) = *(b))
#endif
